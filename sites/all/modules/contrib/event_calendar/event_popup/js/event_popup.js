  ...
  Drupal.behaviors.event_popup = {
    attach: function(context, settings) {
      ...
      if (settings.event_popup.classes) {
        ...
        if (!event.metaKey && !$(this).hasClass('simpleEventProcessed')) {
          ...
          $(this).click(function(event) {
            ...
            var options =  Drupal.event_popup.explodeOptions('width:auto;height:auto;position:[400,240]');
      ...
      if(op) {
        ...
        var url = Drupal.settings.basePath + 'node/add/close-day';
//        var url = Drupal.settings.basePath + 'node/add/' +  node_type;
        var title =  'Закрыть День';
        // Use defaults if not provided
        var selector = 'close-day-node-form';
//        var selector = Drupal.settings.event_popup.selector;
        ...
        if (url && title && selector) {
          ...
          // Use jQuery .get() to request the target page
				...
				$.get(url, {'date':event_date, salon: $tid}, function(data) {
					console.log($( '<div></div>' ).html( data ).find( '#' + selector ).clone())
					 // Re-apply the height if it's auto to accomodate the new content
                if (options.height && options.height == 'auto') {
                  $('#event-popup-container').dialog('option', 'height', options.height);
                }
                ...
                // Attach any behaviors to the loaded content
                //Drupal.attachBehaviors($('#event-popup-container'));	 
                    $popup = $('#event-popup-container').find('button#cancel-popup')
                    $popup.bind('click', function (){
                        $('#event-popup-container').dialog('close');
                    })
				});
				 // Open the dialog
              $('#event-popup-container').dialog('open');
